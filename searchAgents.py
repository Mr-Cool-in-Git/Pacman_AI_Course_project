...
def cornersHeuristic(state, problem):
    ...
    richedCorners = state[1][:]
    restCorners = []

    for i in range(4):
        if not richedCorners[i]:
            restCorners.append(corners[i])

    while len(restCorners) > 0:
        nextCorner = nextPoint(currentPosition, restCorners)
        summary += util.manhattanDistance(currentPosition, nextCorner)
        currentPosition = nextCorner
        sample = list(restCorners)
        sample.remove(nextCorner)
        restCorners = tuple(sample)
    return summary
